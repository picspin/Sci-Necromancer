...
const ISMRMPanel: React.FC = () => {
  ...
  useEffect(() => {
    if (abstractToLoad) {
      setInputText(abstractToLoad.originalText);
      setGeneratedAbstract(abstractToLoad.abstractData);
      setSelectedCategories(abstractToLoad.categories || []);
      setSelectedKeywords(abstractToLoad.keywords);
      setSelectedAbstractType(abstractToLoad.abstractType);
      setImpact(abstractToLoad.abstractData.impact);
      setSynopsis(abstractToLoad.abstractData.synopsis);
      clearLoadedAbstract();
    }
  }, [abstractToLoad, clearLoadedAbstract]);
...
Impact: ${generatedAbstract.impact}
Synopsis: ${generatedAbstract.synopsis}
Keywords: ${generatedAbstract.keywords.join(', ')}`;
      ...
      setError(e instanceof Error ? e.message : 'Failed to perform deep update');
    } finally {
      setIsLoading(false);
    }
  };
              ...
              <div className="space-y-3">
                <div className="flex flex-col sm:flex-row gap-4">
                  {abstractMode === 'standard' ? (
                    ...
                    >
                      <SvgIcon type="sparkles" className="h-5 w-5" />
                      {getMemeTranslation('Generate Creatively', t) ||
                ...
                {generatedAbstract?.abstract && (
                  <button
                    ...
                    disabled={isLoading}
                    className="w-full flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-4 rounded-lg transition-all duration-300 disabled:bg-base-300/50 disabled:cursor-not-allowed animate-fade-in"
          ...
          {activeTab === 'figure' && (
            <div className="space-y-4 animate-fade-in">
              <ImageModeSelector
                ...
                creativeDisabled={!generatedAbstract}
                creativeTooltip={t('tooltips.creative_mode_disabled')}
                t={t}
              />
              {imageMode === 'standard' ? (
                <div className="bg-base-100 p-4 rounded-lg">
                ...
                <div className="p-4 bg-base-100/50 rounded-lg text-center">
                  ...
                  <p className="text-text-secondary font-medium">
                    Creative mode will generate a figure based on the content of your generated
                    abstract.
                  </p>
                </div>
              )}
...
const TabButton: React.FC<{
  ...
  label: string;
  icon: 'text' | 'image';
}> = ({ id, activeTab, setActiveTab, label, icon }) => (
  <button
    onClick={() => setActiveTab(id)}
    className={`flex items-center gap-2 text-sm font-medium py-2 px-4 rounded-t-lg transition-colors duration-200 focus:outline-none ${activeTab === id ? 'border-b-2 border-brand-primary text-brand-primary' : 'text-text-secondary hover:bg-base-300/50'}`}
  >
...
const ModeSelector: React.FC<{
  ...
  <div className="flex bg-base-100 rounded-lg p-1">
    <ModeButton
      label={t('modes.standard_analysis')}
      icon="document"
      active={mode === 'standard'}
      onClick={() => setMode('standard')}
    />
    <ModeButton
      label={getMemeTranslation('Creative Expansion', t) || t('modes.creative_expansion')}
      icon="sparkles"
...
const ImageModeSelector: React.FC<{
  mode: GenerationMode;
  setMode: (mode: GenerationMode) => void;
  creativeDisabled: boolean;
  creativeTooltip?: string;
  t: any;
}> = ({ mode, setMode, creativeDisabled, creativeTooltip, t }) => (
  <div className="flex bg-base-100 rounded-lg p-1">
    <ModeButton
...
const ModeButton: React.FC<{
  ...
  disabled?: boolean;
  tooltip?: string;
}> = ({ label, icon, active, onClick, disabled = false, tooltip }) => (
  <div className="relative w-1/2 group">
    <button
      onClick={onClick}
      disabled={disabled}
      className={`w-full flex items-center justify-center gap-2 py-2 px-3 rounded-md text-sm font-semibold transition-all duration-300 ${active ? 'bg-brand-primary text-white shadow-md' : 'text-text-secondary hover:bg-base-200/50'} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
    >
      <SvgIcon type={icon} className="h-5 w-5" />
      {label}
    </button>
...
const AnalysisStep: React.FC<{
  ...
  return (
    <div className="space-y-4 max-h-[80vh] overflow-y-auto">
      <h2 id="analysis-title" className="text-xl font-bold text-text-primary">
        Analysis Complete ✨
      </h2>
      <p id="analysis-description" className="text-text-secondary">
      ...
      {/* Categories Section */}
      <div>
        <h3 id="categories-heading" className="font-semibold mb-2 text-purple-600">
        ...
        >
          {(Array.isArray(result.categories) ? result.categories : [])
            ...
            .map((cat) => (
              ...
              >
                {cat.name}{' '}
                <span className="text-xs opacity-70">{Math.round(cat.probability * 100)}%</span>
              </button>
            ))}
        </div>
        <p id="categories-help" className="sr-only">
          Use Enter or Space to toggle category selection
        </p>
      </div>

      {/* Keywords Section */}
      <div>
        <h3 id="keywords-heading" className="font-semibold mb-2 text-orange-600">
          Keywords
        </h3>
        <div
          className="flex flex-wrap gap-2"
        ...
        <p id="keywords-help" className="sr-only">
          Use Enter or Space to toggle keyword selection
        </p>
      </div>
...
const TypeSuggestionStep: React.FC<{
  suggestions: AbstractTypeSuggestion[];
  onSelect: (type: AbstractType) => void;
  ...
  const handleKeyDown = (e: React.KeyboardEvent, type: AbstractType, index: number) => {
    if (e.key === 'Enter' || e.key === ' ') {
      ...
      const nextIndex = Math.min(
        index + 1,
        (Array.isArray(suggestions) ? suggestions.length : 1) - 1
      );
  ...
  return (
    <div className="space-y-4">
      <h2 id="type-suggestion-title" className="text-xl font-bold text-text-primary">
        Recommended Abstract Type
      </h2>
      <p id="type-suggestion-description" className="text-text-secondary">
