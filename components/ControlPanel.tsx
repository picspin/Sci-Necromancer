  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.type === 'text/plain') {
        ...
        alert(
          'Only .txt files are supported for direct reading. Please copy and paste content from other file types.'
        );
      }
    }
  };
      ...
      {activeTab === 'text' ? (
        ...
        <div className="space-y-4">
          <ModeSelector
            ...
            creativeDisabled={!isAbstractGenerated}
          ...
          {imageMode === 'standard' ? (
            <div>
              ...
              {imageState.base64 && (
                <img
                  ...
                  alt="Preview"
                  className="mt-4 rounded-lg max-h-40 object-contain"
                />
              )}
            </div>
          ) : (
            <div className="p-4 bg-base-100/50 rounded-lg text-center">
              ...
              <p className="text-text-secondary font-medium">
                Creative mode will generate an image based on the previously generated abstract.
              </p>
            </div>
          )}
          <div>
...
const ModeButton: React.FC<{
  ...
  <div className="relative w-1/2 group">
    ...
    {disabled && tooltip && (
      <span className="absolute bottom-full mb-2 w-max bg-base-300 text-white text-xs rounded py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
        {tooltip}
      </span>
    )}
  </div>
