...
export const ImageGenerationTest: React.FC = () => {
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-lg max-w-2xl mx-auto">
      ...
      {testResult && (
        <div
          className={`mt-6 p-4 rounded-md ${testResult.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}
        >
          ...
          {testResult.details && (
            <div className="mt-4">
              ...
              <div className="bg-gray-50 p-3 rounded text-sm">
                <div>
                  <strong>Platform:</strong> {testResult.details.platform}
                </div>
                <div>
                  <strong>Base URL:</strong> {testResult.details.baseUrl}
                </div>
                <div>
                  <strong>Image Model:</strong> {testResult.details.imageModel}
                </div>
                ...
                {testResult.details.modelCapabilities && (
                  <div className="mt-2">
                    ...
                    <ul className="ml-4 mt-1">
                      <li>
                        Can Generate Images:{' '}
                        {testResult.details.modelCapabilities.canGenerateImages ? '✅' : '❌'}
                      </li>
                      <li>
                        Can Analyze Images:{' '}
                        {testResult.details.modelCapabilities.canAnalyzeImages ? '✅' : '❌'}
                      </li>
                      <li>
                        Is Multi-Modal:{' '}
                        {testResult.details.modelCapabilities.isMultiModal ? '✅' : '❌'}
                      </li>
                      <li>
                        Recommended Approach:{' '}
                        {testResult.details.modelCapabilities.recommendedApproach()}
                      </li>
                    </ul>
                  </div>
                )}

                {testResult.details.availableTools &&
                  testResult.details.availableTools.length > 0 && (
                    <div className="mt-2">
                      <strong>Available Tools:</strong>{' '}
                      {testResult.details.availableTools.join(', ')}
                    </div>
                  )}
