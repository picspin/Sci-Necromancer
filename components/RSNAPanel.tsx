                  ...
                  <button
                    onClick={() => {
                      setSaveTitle(
                        currentAbstractId
                          ? saveTitle
                          : `RSNA Abstract - ${new Date().toLocaleDateString()}`
                      );
          ...
          {activeTab === 'figure' && (
            <div className="space-y-4 animate-fade-in">
              <ImageModeSelector
                ...
                creativeTooltip="Generate an abstract first to enable this mode."
              />
              {imageMode === 'standard' ? (
                <div className="bg-base-100 p-4 rounded-lg">
                  <label
...
const ModeSelector: React.FC<{ mode: GenerationMode; setMode: (mode: GenerationMode) => void }> = ({
  mode,
  setMode,
}) => (
...
}> = ({ mode, setMode, creativeDisabled, creativeTooltip }) => (
  <div className="flex bg-base-100 rounded-lg p-1">
    <ModeButton
      label="Standard Edit"
      icon="document"
...
const RSNAAnalysisStep: React.FC<{
  result: AnalysisResult;
  onConfirm: (cats: Category[], keys: string[]) => void;
}> = ({ result, onConfirm }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-text-secondary">
        Please review and confirm the suggested categories and keywords for your RSNA abstract.
      </p>
