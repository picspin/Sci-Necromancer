...
const AnalysisStep: React.FC<{result: AnalysisResult, onConfirm: (cats: Category[], keys: string[]) => void}> = ({result, onConfirm}) => {
    ...
    return (
        <div className="space-y-4">
            ...
            <div>
                <h3 id="categories-heading" className="font-semibold mb-2">Suggested Categories</h3>
                <div 
            ...
            <div>
                <h3 id="keywords-heading" className="font-semibold mb-2">Suggested Keywords</h3>
                <div 
                    className="flex flex-wrap gap-2" 
                    role="group" 
...
const TypeSuggestionStep: React.FC<{suggestions: AbstractTypeSuggestion[], onSelect: (type: AbstractType) => void}> = ({suggestions, onSelect}) => {
    const handleKeyDown = (e: React.KeyboardEvent, type: AbstractType) => {
        if (e.key === 'Enter' || e.key === ' ') {
            e.preventDefault();
            onSelect(type);
        }
    ...
    return (
        <div className="space-y-4" role="dialog" aria-labelledby="type-suggestion-title" aria-describedby="type-suggestion-description">
            ...
            <div className="space-y-2" role="radiogroup" aria-label="Abstract type selection">
                {suggestions.map(suggestion => (
                    <button 
                        key={suggestion.type} 
                        onClick={() => onSelect(suggestion.type)}
                        onKeyDown={(e) => handleKeyDown(e, suggestion.type)}
                        className="w-full text-left p-3 bg-base-100 hover:bg-base-300/50 border border-base-300 rounded-lg transition-all focus:outline-none focus:ring-3 focus:ring-brand-primary"
                        role="radio"
                        aria-checked="false"
