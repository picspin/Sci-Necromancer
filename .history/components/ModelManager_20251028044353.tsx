...
const ModelManager: React.FC<ModelManagerProps> = ({ onClose }) => {
  ...
  const getStatusIcon = (status?: 'valid' | 'invalid' | 'validating') => {
    if (!status) return null;
    if (status === 'validating') return <SvgIcon type="loader" className="h-4 w-4 animate-spin text-brand-primary" />;
    if (status === 'valid') return <span className="text-green-500">✓</span>;
    if (status === 'invalid') return <span className="text-red-500">✗</span>;
  };
        ...
        {localSettings.provider === 'openai' && (
          <div className="space-y-4 p-4 bg-base-100 rounded-lg">
            ...
            <InputWithValidation
              ...
              helpText="Examples: api.openai.com/v1, openrouter.ai/api/v1, api.siliconflow.cn/v1"
            />
            ...
            <div className="text-xs text-text-secondary space-y-1">
              ...
              <ul className="list-disc list-inside ml-2">
                ...
                <li>OpenRouter: openrouter.ai/api/v1</li>
                <li>SiliconFlow: api.siliconflow.cn/v1</li>
              </ul>
            </div>
          </div>
        )}
