...
const ISMRMPanel: React.FC = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.type !== 'text/plain') {
        setError("Only .txt files can be uploaded directly. For other formats (.pdf, .docx), please copy and paste the text manually.");
        return;
      }
  ...
  const handleGenerateAbstract = async () => {
    ...
    setIsLoading(true);
    setLoadingMessage('Generating final abstract...');
    setError(null);
    setGeneratedAbstract(null);
    try {
        ...
        <div className="flex flex-col gap-6 p-4">
            ...
            <div className="space-y-4 animate-fade-in">
              ...
              {abstractMode === 'standard' ? (
                <div className="bg-base-100 p-4 rounded-lg">
                  ...
                  <input id="file-upload" type="file" accept=".txt,text/plain" onChange={handleFileChange} className="block w-full text-sm text-text-secondary file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-brand-primary/20 file:text-brand-primary hover:file:bg-brand-primary/30"/>
                  <div className="relative mt-2">
                    <textarea value={inputText} onChange={(e) => handleTextChange(e.target.value)} placeholder="Paste the full text of your paper here..." className="w-full h-60 p-3 bg-base-100 border border-base-300 rounded-md focus:ring-2 focus:ring-brand-primary focus:outline-none transition" />
                  </div>
                </div>
              ) : (
                <div className="bg-base-100 p-4 rounded-lg">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                {abstractMode === 'standard' ? (
                  <>
                    <button onClick={handleAnalyze} disabled={isLoading || !inputText.trim()} className="flex-1 flex items-center justify-center gap-2 bg-base-300 hover:bg-opacity-80 text-white font-bold py-3 px-4 rounded-lg transition-all duration-300 disabled:bg-base-300/50 disabled:cursor-not-allowed"><SvgIcon type="sparkles" className="h-5 w-5" />1. Analyze</button>
                    <button onClick={handleGenerateAbstract} disabled={isLoading || !selectedAbstractType} className="flex-1 flex items-center justify-center gap-2 bg-brand-primary hover:bg-brand-secondary text-white font-bold py-3 px-4 rounded-lg transition-all duration-300 disabled:bg-base-300/50 disabled:cursor-not-allowed"><SvgIcon type="document" className="h-5 w-5" />2. Generate</button>
                  </>
                ) : (
                  <button onClick={handleGenerateCreative} disabled={isLoading || !inputText.trim()} className="w-full flex items-center justify-center gap-2 bg-brand-primary hover:bg-brand-secondary text-white font-bold py-3 px-4 rounded-lg transition-all duration-300 disabled:bg-base-300/50 disabled:cursor-not-allowed"><SvgIcon type="sparkles" className="h-5 w-5" />Generate Creatively</button>
                )}
              </div>
            </div>
          )}
          ...
          {activeTab === 'figure' && (
            <div className="space-y-4 animate-fade-in">
                ...
                <button onClick={handleGenerateImage} disabled={isLoading} className="w-full flex items-center justify-center gap-2 bg-brand-primary hover:bg-brand-secondary text-white font-bold py-3 px-4 rounded-lg transition-all duration-300 disabled:bg-base-300/50 disabled:cursor-not-allowed"><SvgIcon type="image" className="h-5 w-5" />Generate Figure</button>
            </div>
          )}
        </div>
...
const AnalysisStep: React.FC<{result: AnalysisResult, onConfirm: (cats: Category[], keys: string[]) => void}> = ({result, onConfirm}) => {
    ...
    return (
        <div className="space-y-4"><h2 className="text-xl font-bold text-text-primary">Analysis Complete</h2><p className="text-text-secondary">Please review and confirm the suggested categories and keywords for your abstract.</p><div><h3 className="font-semibold mb-2">Suggested Categories</h3><div className="flex flex-wrap gap-2">{result.categories.filter(c => c.probability > 0.25).map(cat => (<button key={cat.name} onClick={() => toggleCategory(cat)} className={`px-3 py-1 text-sm font-medium rounded-full border-2 transition-all ${selectedCats.some(c => c.name === cat.name) ? ` ${catBorderColorMap[cat.type]} ${catColorMap[cat.type]} ` : 'border-base-300 bg-base-100 hover:border-brand-primary'}`}>{cat.name} <span className="text-xs opacity-70">({cat.type})</span></button>))}</div></div><div><h3 className="font-semibold mb-2">Suggested Keywords</h3><div className="flex flex-wrap gap-2">{result.keywords.map(key => (<button key={key} onClick={() => toggleKeyword(key)} className={`px-3 py-1 text-sm rounded-full border-2 transition-all ${selectedKeys.includes(key) ? 'border-brand-primary bg-brand-primary/20 text-brand-primary' : 'border-base-300 bg-base-100 hover:border-brand-primary'}`}>{key}</button>))}</div></div><button onClick={() => onConfirm(selectedCats, selectedKeys)} className="w-full bg-brand-primary hover:bg-brand-secondary text-white font-bold py-2 px-4 rounded-lg mt-4">Confirm Selections & Proceed</button></div>
    );
}
