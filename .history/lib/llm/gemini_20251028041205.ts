...
const finalAbstractSchema = {
  ...
  properties: {
    impact: {
      type: Type.STRING,
      description: "A concise, high-impact statement (approx. 50 words) summarizing the key findings and their importance.",
    },
    synopsis: {
      type: Type.STRING,
      description: "A detailed summary of the work (approx. 200-250 words), covering introduction, methods, results, and conclusion.",
    },
    keywords: {
      type: Type.ARRAY,
      items: { type: Type.STRING },
...
export const analyzeContent = async (text: string): Promise<AnalysisResult> => {
  const prompt = prompts.getAnalysisPrompt(text);
  return await callGeminiAPI<AnalysisResult>(prompt, analysisSchema);
};

export const suggestAbstractType = async (text: string, categories: Category[], keywords: string[]): Promise<AbstractTypeSuggestion[]> => {
    const prompt = prompts.getAbstractTypeSuggestionPrompt(text, categories, keywords);
...
export const generateFinalAbstract = async (text: string, type: AbstractType, categories: Category[], keywords: string[]): Promise<AbstractData> => {
    const prompt = prompts.getFinalAbstractPrompt(text, type, categories, keywords);
...
export const generateCreativeAbstract = async (coreIdea: string): Promise<AbstractData> => {
    const prompt = prompts.getCreativeAbstractPrompt(coreIdea);
