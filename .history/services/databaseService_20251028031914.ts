...
export class SupabaseService implements DatabaseService {
    ...
    private async ensureAuthenticated() {
        ...
        if (!user) {
            throw new Error('User not authenticated. Please sign in to use cloud sync.');
        }
        return user;
    }
    ...
    private mergeAbstracts(local: SavedAbstract, remote: SavedAbstract): SavedAbstract {
        // Simple merge strategy: prefer newer fields
        return {
            ...local,
            title: local.updatedAt > remote.updatedAt ? local.title : remote.title,
            abstractData: local.updatedAt > remote.updatedAt ? local.abstractData : remote.abstractData,
            categories: local.updatedAt > remote.updatedAt ? local.categories : remote.categories,
            keywords: local.updatedAt > remote.updatedAt ? local.keywords : remote.keywords,
            updatedAt: new Date()
        };
    }
}
