...
export class JACCModule extends BaseConferenceModule {
  ...
  readonly guidelines: ConferenceGuidelines = {
    abstractTypes: ['JACC Scientific Abstract'],
    wordLimits: {
      abstract: 250,
      impact: 40,
      synopsis: 80,
  ...
  readonly abstractTypes: AbstractType[] = ['JACC Scientific Abstract'];
  ...
  protected async generateConferenceSpecificAbstract(
    ...
    try {
      ...
      throw new Error(
        `JACC abstract generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
