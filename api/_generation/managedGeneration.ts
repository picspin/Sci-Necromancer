...
interface WalletBoundary {
  ...
  ): Promise<{ taskId: string; bonusBalance: number }>;
  continueWorkflow(
    taskId: string,
...
function assertDeliverableOutput(output: ManagedGenerationOutput): void {
...
export async function runManagedGeneration(
  ...
  try {
    ...
    assertDeliverableOutput(output);
    const settlement = await wallet.settleTask(reservation.taskId, true, request.completeWorkflow);
    return { output, bonusBalance: settlement.bonusBalance, workflowId: reservation.taskId };
  } catch (providerError) {
    try {
