...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    const targetQuota = Number(request.body?.targetQuota);
    if (targetQuota !== 100 && targetQuota !== 500) {
    ...
    return response.status(200).json(await member.upgradeAbstractQuota(targetQuota));
  } catch (error) {
