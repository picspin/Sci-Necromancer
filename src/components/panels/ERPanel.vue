        ...
        <div v-if="activeTab === 'abstract'" class="space-y-4 animate-fade-in">
          ...
          <div v-if="abstractMode === 'standard'" class="bg-base-100 p-4 rounded-lg">
            ...
            <div class="relative mt-2">
              <textarea
                ...
                placeholder="Paste your radiology research content for ECR abstract generation..."
          ...
          <div v-else class="bg-base-100 p-4 rounded-lg">
            <label for="creative-prompt" class="block text-sm font-medium text-text-secondary mb-2">
              Creative Expansion: Research Idea for ECR
            </label>
            <input
              id="creative-prompt"
              type="text"
              v-model="inputText"
              @input="handleTextChange"
              placeholder="e.g., Quantitative MRI for early detection of hepatic fibrosis."
    ...
    <Modal v-if="showSaveModal" @close="showSaveModal = false">
      <div class="space-y-4">
        ...
        <p class="text-text-secondary">
          {{
            currentAbstractId
              ? 'Update the title for your ECR abstract.'
              : 'Enter a title for your ECR abstract to save it for later use.'
          }}
        </p>
        <div>
...
import { type ResearchTypeGuideline } from '@/lib/conference/modules/ERModule';
...
const ecrAbstractTypes: AbstractTypeSuggestion[] = [
  { type: 'ECR Research Presentation', probability: 1.0 },
  { type: 'ECR Clinical Trials in Radiology', probability: 0.8 },
  { type: 'ECR EPOS Scientific Poster', probability: 0.9 },
  { type: 'ECR EPOS Educational Poster', probability: 0.7 },
  { type: 'ECR Student Presentation', probability: 0.6 },
];
...
const handleAnalyze = async () => {
  ...
  isLoading.value = true;
  loadingMessage.value = 'Analyzing content for ECR submission...';
  error.value = null;
  resetWorkflow();
  try {
    const result = await llm.analyzeContentForConference(inputText.value, 'ER');
...
const handleAnalysisConfirmation = async (
  cats: Category[],
  keys: string[],
  researchType: ResearchTypeGuideline | null
) => {
  selectedCategories.value = cats;
  selectedKeywords.value = keys;
  selectedResearchType.value = researchType;
  isLoading.value = true;
  loadingMessage.value = 'Suggesting ECR abstract types...';
  try {
    // Filter suggestions based on research type
    let suggestions = [...ecrAbstractTypes];

    // Boost Clinical Trials type for RCTs
    if (researchType?.checklist === 'CONSORT') {
      suggestions = suggestions.map((s) =>
        s.type === 'ECR Clinical Trials in Radiology' ? { ...s, probability: 1.0 } : s
      );
    }
...
const handleGenerateAbstract = async () => {
  ...
  isLoading.value = true;
  loadingMessage.value = 'Generating ECR abstract...';
  error.value = null;
  generatedAbstract.value = null;
  try {
...
const handleGenerateCreative = async () => {
  ...
  isLoading.value = true;
  loadingMessage.value = 'Creatively generating ECR abstract...';
  error.value = null;
  resetWorkflow();
  try {
    const result = await llm.generateCreativeAbstractForConference(inputText.value, 'ER');
    generatedAbstract.value = result;
    selectedKeywords.value = result.keywords;
  } catch (e) {
...
const openSaveModal = () => {
  if (!currentAbstractId.value) {
    saveTitle.value = `ECR Abstract - ${new Date().toLocaleDateString()}`;
  }
...
watch(abstractToLoad, (newAbstract) => {
  if (newAbstract && newAbstract.conference === 'ER') {
