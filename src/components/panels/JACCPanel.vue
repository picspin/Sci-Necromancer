  ...
  <div>
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div v-if="activeTab === 'figure'" class="space-y-4 animate-fade-in">
          ...
          <div v-else class="p-4 bg-base-100/50 rounded-lg text-center">
            ...
            <p class="text-text-secondary font-medium">
              Creative mode will generate a figure based on your JACC abstract content.
            </p>
          </div>
          <div class="bg-base-100 p-4 rounded-lg">
            ...
            <textarea
              ...
              placeholder="e.g., 'Highlight coronary arteries', 'Add echocardiography measurements', 'Create before/after comparison'."
      ...
      <OutputDisplay
        ...
        conference="JACC"
        :abstract-type="selectedAbstractType || 'JACC Scientific Abstract'"
      />
    </div>
    ...
    <Modal v-if="showSaveModal" @close="showSaveModal = false">
      <div class="space-y-4">
        ...
        <p class="text-text-secondary">
          {{
            currentAbstractId
              ? 'Update the title for your JACC abstract.'
              : 'Enter a title for your JACC abstract to save it for later use.'
          }}
        </p>
        <div>
...
import JACCAnalysisStep from './JACCPanelComponents/JACCAnalysisStep.vue';
...
const handleAnalyze = async () => {
  ...
  isLoading.value = true;
  loadingMessage.value = 'Analyzing content for JACC submission...';
  error.value = null;
  resetWorkflow();
  try {
    // Use JACC-specific analysis
    const result = await llm.analyzeContentForConference(inputText.value, 'JACC');
...
const handleAnalysisConfirmation = async (cats: Category[], keys: string[]) => {
  ...
  isLoading.value = true;
  loadingMessage.value = 'Suggesting JACC abstract types...';
  try {
...
const handleGenerateAbstract = async () => {
  ...
  isLoading.value = true;
  loadingMessage.value = 'Generating JACC scientific abstract...';
  error.value = null;
  generatedAbstract.value = null;
  try {
...
const handleGenerateCreative = async () => {
  ...
  isLoading.value = true;
  loadingMessage.value = 'Creatively generating JACC abstract...';
  error.value = null;
  resetWorkflow();
  try {
    const result = await llm.generateCreativeAbstractForConference(inputText.value, 'JACC');
    generatedAbstract.value = result;
    selectedKeywords.value = result.keywords;
  } catch (e) {
...
const openSaveModal = () => {
  if (!currentAbstractId.value) {
    saveTitle.value = `JACC Abstract - ${new Date().toLocaleDateString()}`;
  }
...
watch(abstractToLoad, (newAbstract) => {
  if (newAbstract && newAbstract.conference === 'JACC') {
