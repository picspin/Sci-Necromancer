<template>
  ...
  >
    <div class="space-y-4">
      <!-- Search -->
      <div class="relative">
        <label for="help-search" class="sr-only"> Search help documentation </label>
        <input
          id="help-search"
          v-model="searchQuery"
          type="text"
          placeholder="Search help topics..."
          class="w-full px-4 py-2 pl-10 bg-base-100 border border-base-300 rounded-md focus:ring-2 focus:ring-brand-primary focus:outline-none"
          aria-label="Search help documentation"
        />
        <SvgIcon
          type="document"
          class="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-text-secondary"
          aria-hidden="true"
        />
      </div>
      ...
      <div v-if="currentSection" class="space-y-4">
        <button
          ...
          class="flex items-center gap-2 text-brand-primary hover:underline focus:outline-none focus:ring-3 focus:ring-brand-primary rounded-md"
        ...
        <div>
          ...
          <div class="prose prose-invert max-w-none text-text-secondary whitespace-pre-line">
      ...
      <!-- Topics List -->
      <div v-else class="space-y-2" role="list" aria-label="Help topics">
        <template v-if="filteredSections.length > 0">
          <button
            v-for="section in filteredSections"
            :key="section.id"
            @click="selectedSection = section.id"
            class="w-full text-left p-4 bg-base-100 hover:bg-base-300/50 border border-base-300 rounded-lg transition-all focus:outline-none focus:ring-3 focus:ring-brand-primary"
            role="listitem"
        ...
        <div v-else class="text-center py-8 text-text-secondary">
          <SvgIcon type="info" class="h-12 w-12 mx-auto mb-3 opacity-50" aria-hidden="true" />
          <p>No help topics found matching "{{ searchQuery }}"</p>
          <button
            @click="searchQuery = ''"
            class="mt-2 text-brand-primary hover:underline focus:outline-none focus:ring-3 focus:ring-brand-primary rounded-md"
          >
            Clear search
          </button>
        </div>
      ...
      <!-- Quick Links -->
      <div v-if="!currentSection && !searchQuery" class="pt-4 border-t border-base-300">
        <h4 class="text-sm font-semibold text-text-primary mb-2">Quick Links</h4>
        <div class="flex flex-wrap gap-2">
          <button
            v-for="id in quickLinkIds"
            :key="id"
            @click="selectedSection = id"
            class="px-3 py-1 text-sm bg-brand-primary/20 text-brand-primary rounded-full hover:bg-brand-primary/30 transition-colors focus:outline-none focus:ring-3 focus:ring-brand-primary"
            :aria-label="`Quick link to ${helpSections.find((s) => s.id === id)?.title}`"
          >
            {{ helpSections.find((s) => s.id === id)?.title }}
          </button>
        </div>
      </div>
    </div>
...
const quickLinkIds = ['getting-started', 'keyboard-shortcuts', 'accessibility', 'troubleshooting'];

const filteredSections = computed(() => {
  if (!searchQuery.value) return helpSections;
...
const currentSection = computed(() => {
  return selectedSection.value ? helpSections.find((s) => s.id === selectedSection.value) : null;
});
</script>
